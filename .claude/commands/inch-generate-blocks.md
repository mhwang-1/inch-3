...
digraph inch_generate_blocks {
    ...
    subgraph cluster_gen_loop {
        ...
        "Self-check l1_text naturalness:\nFor each block, re-read the l1_text (before any parenthetical\nannotation).\nConsult knowledge/{lang_code}.md section '## L1 Register Target'\nfor the profile's target voice AND any L1-specific anti-patterns\nrecorded there (e.g. for Mandarin L1: 的-chain pileup, 所-被-由\nliterary passive, four-character compound stacking;\nfor English L1: translationese, awkward preposition stranding; etc.).\nDoes the draft read naturally in that register, or does it sound\nlike a machine gloss? If stilted, rephrase before writing to DB.\nConsult the bad→good examples table in {lang_code}.md if present.\nNEVER hard-code L1-specific rules here — they live in the\nper-profile knowledge file only." [shape=box, style=filled, fillcolor=lightyellow];

        "Generate blocks following NP (Phase 1) then VP (Phase 2)\nthen combining (Phase 3) per chunking-strategy.md.\nUse {lang_code}.md for language-specific pattern rules.\nEach block: tts_text, l2_display (= tts_text),\nl1_text (L1 translation + optional usage notes), role." -> "Self-check l1_text naturalness:\nFor each block, re-read the l1_text (before any parenthetical\nannotation).\nConsult knowledge/{lang_code}.md section '## L1 Register Target'\nfor the profile's target voice AND any L1-specific anti-patterns\nrecorded there (e.g. for Mandarin L1: 的-chain pileup, 所-被-由\nliterary passive, four-character compound stacking;\nfor English L1: translationese, awkward preposition stranding; etc.).\nDoes the draft read naturally in that register, or does it sound\nlike a machine gloss? If stilted, rephrase before writing to DB.\nConsult the bad→good examples table in {lang_code}.md if present.\nNEVER hard-code L1-specific rules here — they live in the\nper-profile knowledge file only.";
        ...
        "Self-check l1_text naturalness:\nFor each block, re-read the l1_text (before any parenthetical\nannotation).\nConsult knowledge/{lang_code}.md section '## L1 Register Target'\nfor the profile's target voice AND any L1-specific anti-patterns\nrecorded there (e.g. for Mandarin L1: 的-chain pileup, 所-被-由\nliterary passive, four-character compound stacking;\nfor English L1: translationese, awkward preposition stranding; etc.).\nDoes the draft read naturally in that register, or does it sound\nlike a machine gloss? If stilted, rephrase before writing to DB.\nConsult the bad→good examples table in {lang_code}.md if present.\nNEVER hard-code L1-specific rules here — they live in the\nper-profile knowledge file only." -> "Accumulate blocks for this sentence in memory.\nDo NOT write to DB yet — all blocks across\nall sentences are batched into a single transaction.";
    ...
    subgraph cluster_rules {
        ...
        "L1 translations must match the profile's register target.\nLiteral glosses are failures even when technically accurate.\nThe register target is defined per profile in\nknowledge/{lang_code}.md under '## L1 Register Target' —\nNEVER assume Mandarin or any other specific L1." [shape=octagon, style=filled, fillcolor=orange];
        "PHASE 0 OUTPUT TEMPLATE (required format, printed verbatim\nfor EVERY sentence during Phase 0):\n\n[Sentence M/N | sentence_id=XXX]\nL2: {full l2_text}\n\n1. Main clause structure: {brief tag, e.g. 'SV copula', 'VP-only'}\n2. Subject NP: {span} → tentative pattern: {NP-A/B(a)/B(b)/Compound}\n3. Predicate VP: {span} → tentative pattern: {VP-A through VP-I}\n4. Fixed expressions: [{expr1}, {expr2}]  (empty list if none)\n\n5. Planned blocks:\n   - block 0: '{tts_text}'  role={ROLE}   l1='{draft}'\n   - block 1: '{tts_text}'  role={ROLE}   l1='{draft}'\n   - …\n   - block N-1: '{full sentence}'  role=Compound   l1='{draft}'\n\n6. Absolute rule check:\n   [x] Block 0 is NOT the full sentence\n   [x] No fixed expression split across blocks\n   [x] l2_display == tts_text for every block\n   [x] Final block = full sentence" [shape=octagon, style=filled, fillcolor=lightyellow];
    }
}
```
